...
// ── Modal confirmation depuis WhatsApp ────────────────────────────────────────
const NotifModal = ({ notifModal, onClose, onConfirmer, onSaisir }) => {
    const isMed = notifModal.type === 'medicament';
    return (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center' }}>
            <div style={{ background:'#fff', borderRadius:20, padding:32, width:340, maxWidth:'90vw', textAlign:'center', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
                <div style={{ fontSize:48, marginBottom:12 }}>{isMed ? '💊' : '📊'}</div>
                <h4 style={{ fontWeight:700, marginBottom:8 }}>
                    {isMed ? 'Avez-vous pris votre médicament ?' : 'Avez-vous fait votre mesure ?'}
                </h4>
                <p style={{ color:'#64748b', fontSize:14, marginBottom:24 }}>
                    {isMed ? 'Confirmez la prise pour mettre à jour votre suivi.' : 'Voulez-vous saisir votre mesure maintenant ?'}
                </p>
                <div style={{ display:'flex', gap:10 }}>
                    <button onClick={onClose} style={{ flex:1, border:'1.5px solid #e2e8f0', borderRadius:12, padding:'12px 0', fontSize:14, fontWeight:600, cursor:'pointer', background:'#fff', color:'#64748b' }}>
                        ⏰ Plus tard
                    </button>
                    <button onClick={isMed ? onConfirmer : onSaisir}
                        style={{ flex:2, border:'none', borderRadius:12, padding:'12px 0', fontSize:14, fontWeight:700, cursor:'pointer', background: isMed ? '#1cc88a' : '#4e73df', color:'#fff' }}>
...
const SaisieModal = ({ mesure, onClose, onSave }) => {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await onSave(mesure.id, value, note, mesure.disease_name);
            onClose();
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center' }}
            ...
            <div style={{ background:'#fff', borderRadius:20, width:400, maxWidth:'92vw', overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}>
                ...
                <div style={{ padding:'20px 24px' }}>
                    ...
                    <input type="number" placeholder={mesure.unit ? `ex: 1.2 ${mesure.unit}` : 'ex: 120'}
                    ...
                    <textarea placeholder="Ex : à jeun, après exercice…" value={note} onChange={e => setNote(e.target.value)} rows={2}
...
const Dashboard = () => {
    ...
    const [notifModal,     setNotifModal]     = useState(null); // ← modal WhatsApp
    ...
    // ── Lire les params URL depuis le lien WhatsApp ───────────────────────────
    useEffect(() => {
    ...
    // ── Confirmer prise médicament depuis modal WhatsApp ──────────────────────
    const handleConfirmerPrise = async () => {
        try {
            await axios.post(`${API}/api/medicaments/${notifModal.takeId}/confirmer`, {}, { headers: authHeaders() });
    ...
    // ── Ouvrir modal saisie mesure depuis modal WhatsApp ──────────────────────
    const handleSaisirMesure = async () => {
        try {
            const res = await axios.get(`${API}/api/measure-take/${notifModal.takeId}`, { headers: authHeaders() });
            setSaisieModal(res.data);
        } catch { toast.error('Erreur serveur'); }
        setNotifModal(null);
    };

    // ── Marquer médicament comme pris depuis le dashboard ────────────────────
    const markMedDone = useCallback(async (takeId) => {
        setDateCache(prev => {
            ...
            return {
                ...
                [todayISO]: {
                    ...cached,
                    medications: (cached.medications || []).map(med => ({
                        ...med,
                        takes: (med.takes || []).map(t => t.id === takeId ? { ...t, status: 'done' } : t),
                    })),
                },
            };
        });
        try {
            await axios.post(`${API}/api/medicaments/${takeId}/confirmer`, {}, { headers: authHeaders() });
    ...
    // ── Sauvegarder mesure + envoyer WhatsApp ────────────────────────────────
    const saveMeasure = useCallback(async (measureId, value, note, nomMesure) => {
        await axios.post(`${API}/api/measures/result`, { measure_id: measureId, value, note }, { headers: authHeaders() });
        await axios.post(`${API}/api/measures/result/confirm`,
            { nom: nomMesure ?? 'Mesure' },
            { headers: authHeaders() }
        );
    ...
    const medTakes = useMemo(() => {
        ...
        return meds
            ...
            .flatMap(med => (med.takes || []).map(take => ({
                ...take, _type: 'med', _name: med.medication_name,
                _image: med.image, _color: med.reminder_color ?? '#4e73df', _med: med,
            })))
            .sort(compareTime);
    ...
    const measureTakes = useMemo(() => {
        ...
        return measures
            .filter(mes => shouldTakeOnDate({ frequency_type: mes.frequency_type, frequency_days: mes.frequency_days, start_day: mes.start_day }, activeFullDate))
            .flatMap(mes => (mes.takes || []).map(take => ({
                ...take, _type: 'measure', _name: mes.disease_name,
                _color: sevColor(mes.severity), _severity: mes.severity, _unit: mes.unit, _mes: mes,
            })))
            .sort(compareTime);
    }, [activeDateData, activeFullDate, isFutureDate]);
    ...
    const observance = useMemo(() => {
        const medT = (todayData.medications || []).filter(m => shouldTakeOnDate(m, todayISO)).flatMap(m => m.takes || []);
        const mesT = (todayData.measures    || []).filter(m => shouldTakeOnDate({ frequency_type: m.frequency_type, frequency_days: m.frequency_days, start_day: m.start_day }, todayISO)).flatMap(m => m.takes || []);
        const all  = [...medT, ...mesT];
        return all.length === 0 ? '—' : Math.round((all.filter(t => t.status === 'done').length / all.length) * 100) + '%';
    }, [todayData, todayISO]);
    ...
    return (
        <div className="container-fluid min-vh-100 bg-light py-4 px-xl-5" style={{ fontFamily: "'Inter', sans-serif" }}>
            ...
            <section className="mb-4">
                <div className="card border-0 shadow-sm rounded-4 p-2 bg-white">
                    ...
                    <p className="text-center text-muted mb-0 mt-1" style={{ fontSize:'0.7rem' }}>
                        {weekDays[0]?.monthLabel}
                        <span className="ms-2 badge bg-primary rounded-pill" style={{ fontSize:'0.6rem' }}>Cette semaine</span>
                    </p>
                </div>
            </section>
                ...
                <div className="col-lg-8">
                    <div className="card border-0 shadow-sm rounded-4 p-4 bg-white">
                        <div className="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
                            ...
                            <span className="badge bg-primary rounded-pill">{allTakes.length} rappel{allTakes.length !== 1 ? 's' : ''}</span>
                        </div>

                        {activeDayStats.total > 0 && (
                            <div className="mb-3">
                                <div className="progress rounded-pill" style={{ height:5 }}>
                                    <div className="progress-bar rounded-pill" style={{ width:`${activeDayStats.pct ?? 0}%`, background: pctColor(activeDayStats.pct ?? 0), transition:'width 0.5s' }} />
                                </div>
                            </div>
                        )}
                        ...
                        <div className="d-flex gap-2 mb-3 flex-wrap">
                            {[
                                ...
                                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                                    ...
                                    style={{ background: activeTab === tab.key ? '#4e73df' : '#f1f5f9', color: activeTab === tab.key ? '#fff' : '#64748b', border:'none', fontWeight:600, fontSize:12, padding:'5px 14px', transition:'all 0.15s' }}>
                                    <i className={`bi ${tab.icon}`} />
                                    {tab.label}
                                    <span style={{ background: activeTab === tab.key ? 'rgba(255,255,255,0.25)' : '#e2e8f0', color: activeTab === tab.key ? '#fff' : '#64748b', borderRadius:99, fontSize:10, padding:'1px 7px', fontWeight:700 }}>
                                        {tab.count}
                                    </span>
                                </button>
                            ))}
                            ...
                            <>
                                <div className="d-flex flex-column gap-2">
                                    {pagedTakes.map((take, j) => {
                                        ...
                                        const borderColor = isDone ? '#1cc88a' : isMissed ? '#e74a3b' : isLate ? '#f6a935' : take._color;
                                            ...
                                            <div key={`${take.id}-${j}`}
                                                ...
                                                {isToday && !isDone ? (
                                                    isFuture ? (
                                                        ...
                                                        <button onClick={() => setSaisieModal(take._mes)}
                                                        ...
                                                        <button onClick={() => markMedDone(take.id)}
                                    ...
                                    <div className="d-flex align-items-center justify-content-between mt-4 pt-3" style={{ borderTop:'1px solid #f1f5f9' }}>
                                        ...
                                        <div className="d-flex gap-2 align-items-center">
                                            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                                                className="btn btn-sm btn-light rounded-circle" style={{ width:32, height:32, padding:0, opacity: page === 1 ? 0.4 : 1 }}>
                                                <i className="bi bi-chevron-left" style={{ fontSize:12 }} />
                                            </button>
                                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                                                <button key={p} onClick={() => setPage(p)}
                                                    className="btn btn-sm rounded-circle"
                                                    style={{ width:32, height:32, padding:0, background: page === p ? '#4e73df' : '#f1f5f9', color: page === p ? '#fff' : '#64748b', border:'none', fontWeight: page === p ? 700 : 400, fontSize:13 }}>
                                                    {p}
                                                </button>
                                            ))}
                                            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                                                className="btn btn-sm btn-light rounded-circle" style={{ width:32, height:32, padding:0, opacity: page === totalPages ? 0.4 : 1 }}>
            ...
            <div className="position-fixed" style={{ bottom:30, right:30, zIndex:1050 }}>
                {fabOpen && (
                    <div className="d-flex flex-column gap-2 mb-3 align-items-end">
                        {[
                            { label:'Ajouter un médicament', icon:'bi-capsule',          path:'/Medicament', bg:'bg-primary' },
                            { label:'Ajouter une mesure',    icon:'bi-clipboard2-pulse', path:'/Mesure',     bg:'',          style:{ background:'#6f42c1' } },
                        ].map((item, i) => (
                            <button key={i} onClick={() => { navigate(item.path); setFabOpen(false); }}
                                className="btn bg-white shadow rounded-pill px-4 py-2 fw-bold d-flex align-items-center gap-2 border-0">
                                <span className={`rounded-circle d-flex align-items-center justify-content-center text-white ${item.bg}`}
                                    style={{ width:28, height:28, ...(item.style ?? {}) }}>
                                    <i className={`bi ${item.icon}`} style={{ fontSize:12 }} />
                                </span>
                                {item.label}
                            </button>
                        ))}
            ...
            {/* ── Modals ── */}
            {saisieModal && (
                <SaisieModal mesure={saisieModal} onClose={() => setSaisieModal(null)} onSave={saveMeasure} />
            )}
            {notifModal && (
                <NotifModal
